...
__attribute__((unused)) char _verify_const_msg[__builtin_constant_p(logmsg) ? 1 : -1]; \
__attribute__((section("__TEXT,__os_trace"))) static const char _m[] = logmsg; \
OS_CONCAT(_os_trace_debug_with_payload,OS_COUNT_ARGS(__VA_ARGS__))(_m, OS_TRACE_TYPE_DEBUG, ##__VA_ARGS__); \
} while (0)

#define os_trace_error_with_payload(logmsg, ...) do { \
__attribute__((unused)) char _verify_const_msg[__builtin_constant_p(logmsg) ? 1 : -1]; \
__attribute__((section("__TEXT,__os_trace"))) static const char _m[] = logmsg; \
OS_CONCAT(_os_trace_error_with_payload,OS_COUNT_ARGS(__VA_ARGS__))(_m, OS_TRACE_TYPE_ERROR, ##__VA_ARGS__); \
} while (0)

#define os_trace_fault_with_payload(logmsg, ...) do { \
	__attribute__((unused)) char _verify_const_msg[__builtin_constant_p(logmsg) ? 1 : -1]; \
	__attribute__((section("__TEXT,__os_trace"))) static const char _m[] = logmsg; \
	OS_CONCAT(_os_trace_fault_with_payload,OS_COUNT_ARGS(__VA_ARGS__))(_m, OS_TRACE_TYPE_FAULT, ##__VA_ARGS__); \
} while (0)
